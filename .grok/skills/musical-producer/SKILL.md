...
- **Selection is real hummingbirds.** One chip per color, forever: Ruby=`hum_sel_00`, Gold=`01`, Teal=`02`, Violet=`03`, Peach=`04`. `Sfx.Chirp(color)` only, and only on select. Hops are a soft cartoon fwip-fwip (`Sfx.FlockFlutter`): five round whooshes with falling pitch, no grit, staggered ~90ms. Never another chirp on a hop.
...
- **Selection chips:** one chip per color forever (`Sfx.Chirp(color)` / `Audio/Select`): Ruby=`hum_sel_00`, Gold=`01`, Teal=`02`, Violet=`03`, Peach=`04`. Every red bird is the same chip. Moves are loud flutter, not chips. Do not synthesize replacements. Do not pitch them up.
...
- **Combo:** two or more feeders paid in one move (`Sfx.Combo`). Wood-pluck D-major jingle only (`MakeComboJingle`) — never stacked gongs, never an announcer, never extra leave whooshes. MixDesk `ComboWarm` fades in quiet pizzicato fifths on the Bed seat, recedes ~4s after last collect, ≤ 0.04 × duck. Harder camera punch is fine; do not add a second melody.
...
- **Feeder leave:** 12 satisfying receding whooshes (`Sfx.FeederLeave` / `Audio/Whoosh`). Unhook + falling swoop + silk air. Motion of an object leaving, never coarse blowing wind, never a bell. `PullAway` uses leave; combo does not stack leave whooshes.
